...
(function () {
	...
	_cfg.qn = {
		getUploadTokenApi: 'http://www.char-set.com.cn/api/getUploadToken', //上传到指定目录
		getUploadTokenApi2: 'http://www.char-set.com.cn/api/getUploadToken2', //上传到随机文件名
		BucketDomain: 'http://ojc83pmmg.bkt.clouddn.com/'
	};
	...
	if (window.location.host == 'www.char-set.com.cn') {
		_cfg.home = 'http://' + window.location.host + '/web/'
	};
	...
	*/
	 ...
	 * @returns {int} uploadId整数，指向一个数组包含所有文件的xhr，数组存放在_cfg.xhrs[uploadId]
	 */
	_fns.uploadFile2 = function (type, btnjo, beforefn, progressfn, successfn, abortfn, errorfn, completefn, domain, multi) {
		if (!btnjo) {
			__errhdlr(new Error('_fns.uploadFile:button undefined.'));
			return;
		};
		...
		filejo.remove();
		if (1 == type) {
			filejo = $('<input id="uploadFileInput" type="file" accept="image/gif, image/jpeg,image/png" style="display:none"></input>').appendTo(btnjo);
		} else {
			filejo = $('<input id="uploadFileInput" type="file" style="display:none"></input>').appendTo(btnjo);
		}
		if (multi) filejo.attr('multiple', "multiple");
